...
with st.form("rocket_launch_form"):
    ...
    for i in range(1, 5):
        ...
        with st.expander(f"**Dropzone {i}**", expanded=(i == 1)):
            ...
            st.caption("**Vertices** (minimum 3 recommended)")
            ...
            for idx in range(len(current_verts)):
                st.text_input(
                    ...
                    key=f"{dz}_vert_{idx}",
                    placeholder="e.g. 14°35'12\"N 120°12'45\"E"
                )
            ...
            with col_add:
                if st.button("➕ Add Vertex", key=f"add_v_{dz}", use_container_width=True):
                    if len(current_verts) < 10:          # safety limit
    ...
    # Optional: quick validation
        for dz_id, dz in dropzones.items():
            valid_verts = [p for p in dz["vertices"] if p.strip()]
...
if submitted:
    ...
    # Convert and pad to exactly 8 columns (what CAAP expects)
    dz_compact = []
    for i in range(1, 5):
        verts = dropzones[f"DZ{i}"]["vertices"]
        compact_verts = [convert_to_compact(c) for c in verts]
        compact_verts += [""] * (8 - len(compact_verts))   # pad
        dz_compact.append(compact_verts[:8])               # never more than 8
